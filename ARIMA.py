...
if __name__ == "__main__":
    ...
    # 1️⃣ Simulazione di una serie ARIMA(2,1,1)
    np.random.seed(0)
    n = 300
    ...
    for t in range(2, n):
        # Processo ARMA(2,1)
        series[t] = (
            0.6 * series[t - 1] - 0.3 * series[t - 2] + eps[t] + 0.4 * eps[t - 1]
        )
    # Differenziazione inversa → serie ARIMA(2,1,1)
    series = np.cumsum(series)

    # 2️⃣ Inizializza e stima il modello
    model = ARIMA(series, p=2, d=1, q=1)
    model.fit()

    # 3️⃣ Previsioni future
    ...
    # ACF e PACF (metodi ereditati da TimeSeries)
    print("Plot ACF e PACF della serie originale...")
    model.plot_ACF(max_lag=20)
    model.plot_PACF(max_lag=20)

    # Fitted vs Actual sulla serie differenziata
    print("Plot dei valori stimati (in-sample) vs reali (differenziati)...")
    model.plot_fitted_vs_actual()

    # Residui
    print("Plot dei residui del modello...")
    model.plot_residuals()

    # Previsioni future
    print("Plot delle previsioni future...")
